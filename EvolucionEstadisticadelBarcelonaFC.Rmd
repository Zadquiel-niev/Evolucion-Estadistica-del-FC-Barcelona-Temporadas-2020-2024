---
title: "ANÁLISIS ESTADÍSTICO DEL FC BARCELONA EN LAS TEMPORADAS 2020-2024"
...
barca_data <- matches_full_la_liga %>% 
  ...
  filter(!grepl("^200", formation)) # datos problematicos
...
azul_principal <- "#004D98"
azul_medio <- "#1E88E5"
azul_claro <- "#64B5F6"
azul_oscuro <- "#0D47A1"
gris_contraste <- "#424242"

g1 <- ggplot(tendencias_anuales, aes(x = as.factor(season))) +
  ...
  geom_ribbon(aes(ymin = promedio_gf - sd_gf, ymax = promedio_gf + sd_gf, group = 1), 
              fill = azul_principal, alpha = 0.2) +
  geom_ribbon(aes(ymin = promedio_ga - sd_ga, ymax = promedio_ga + sd_ga, group = 1), 
              fill = azul_medio, alpha = 0.2) +
  geom_point(aes(y = promedio_gf), size = 3, color = azul_principal) +
  geom_point(aes(y = promedio_ga), size = 3, color = azul_medio) +
  labs(title = "Evolución Ofensiva vs. Defensiva con Intervalos de Variabilidad",
       subtitle = "Líneas sólidas: promedios. Áreas sombreadas: ±1 desviación estándar",
       x = "Temporada", y = "Goles por Partido",
       color = "Métrica") +
  scale_color_manual(values = c("Goles a Favor" = azul_principal, 
                                "Goles en Contra" = azul_medio)) +
  theme_minimal() +
...
Interpretación Gráfico 1:Este gráfico muestra el rendimiento del equipo. La línea de Goles a Favor se mantuvo cerca de 2 goles por partido. En 2022, esta cantidad bajó ligeramente. La línea de Goles en Contra mejoró con el tiempo. Después de una temporada, se mantuvo en un nivel más bajo. Las áreas sombreadas alrededor de esta línea se hicieron más estrechas entre 2022 y 2024. Esto significa que la defensa del equipo fue más constante en las últimas temporadas.

```{r grafico2, echo=FALSE, message=FALSE, warning=FALSE}

g2 <- ggplot(tendencias_anuales, aes(x = as.factor(season))) +
  geom_col(aes(y = promedio_posesion, fill = "Posesión"), alpha = 0.8) +
  geom_errorbar(aes(ymin = promedio_posesion - sd_posesion, 
                    ymax = promedio_posesion + sd_posesion), 
                width = 0.3, color = azul_oscuro) +
  geom_line(aes(y = efectividad/2, color = "Efectividad", group = 1), size = 1.5) +
  geom_point(aes(y = efectividad/2), size = 3, color = azul_claro) +
  scale_y_continuous(
    name = "Posesión (%)",
    sec.axis = sec_axis(~.*2, name = "Efectividad (%)")
  ) +
  labs(title = "Posesión y Efectividad por Temporada",
       subtitle = "Barras: posesión promedio. Línea: porcentaje de efectividad",
       x = "Temporada") +
  scale_fill_manual(values = c("Posesión" = azul_principal)) +
  scale_color_manual(values = c("Efectividad" = azul_claro)) +
  theme_minimal() +
  theme(legend.position = "bottom")
...
Interpretación Gráfico 2: El gráfico muestra que tener la posesión del balón no indica cuán efectivo es un equipo para anotar puntos. La barra de posesión se mantuvo alta y estable, entre 64% y 67%. La línea de efectividad tuvo cambios durante la temporada. En 2023, el equipo tuvo su mejor rendimiento. En ese año, lograron la máxima cantidad de puntos por partido. La posesión del balón en 2023 fue similar a la de otras temporadas. Esto indica que los mejores resultados no dependieron solo del control del balón. En cambio, mejoraron en cómo usaron esa posesión para anotar. La eficiencia en las jugadas y en finalizar las acciones fue mayor en 2023. Por lo tanto, la clave para mejorar fue la estrategia y la ejecución, no solo tener más posesión.

```{r grafico3, echo=FALSE, message=FALSE, warning=FALSE}

barca_data <- barca_data %>%
  mutate(puntos = ifelse(result == "W", 3, ifelse(result == "D", 1, 0)))

g3 <- ggplot(barca_data, aes(x = venue, y = puntos, fill = venue)) +
  geom_boxplot(alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.5, size = 1.5) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white") +
  labs(title = "Distribución de Puntos por Partido según Localía",
       x = "Localía", y = "Puntos") +
  scale_fill_manual(values = c("Home" = azul_principal, "Away" = azul_medio)) +
  scale_x_discrete(labels = c("Home" = "Casa", "Away" = "Fuera")) +
  theme_minimal() +
  theme(legend.position = "none")

g4 <- ggplot(barca_data, aes(x = venue, y = gf, fill = venue)) +
  geom_boxplot(alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.5, size = 1.5) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white") +
  labs(title = "Distribución de Goles por Partido según Localía",
       x = "Localía", y = "Goles") +
  scale_fill_manual(values = c("Home" = azul_principal, "Away" = azul_medio)) +
  scale_x_discrete(labels = c("Home" = "Casa", "Away" = "Fuera")) +
  theme_minimal() +
...
Interpretación Gráfico 3 (Puntos): El boxplot muestra que la mediana de puntos por partido es 3 en casa y fuera. Esto indica que la mayoría de los resultados en ambos casos son victorias. La dispersión de los resultados es mayor cuando el equipo juega fuera. Como visitante, el equipo obtiene más resultados de 1 punto, que son empates. Además, hay más partidos con 0 puntos, que son derrotas o bajo rendimiento.

Interpretación Gráfico 4 (Goles):El rendimiento ofensivo del Barcelona es mejor cuando juega en casa. La cantidad de goles por partido tiene una mediana similar en ambos casos, aproximadamente 2 goles. El boxplot de "Casa" es más compacto. Esto muestra que el equipo tiene una producción goleadora más constante en casa. Fuera de casa, los resultados varían más. Hay más partidos con poca o ninguna producción ofensiva. Esto indica que jugar en casa ayuda a que el ataque sea más eficiente.

```{r grafico4, echo=FALSE}

g5 <- ggplot(formaciones_analisis, aes(x = reorder(formation, media_puntos), y = media_puntos)) +
  geom_col(aes(fill = media_puntos), alpha = 0.8, width = 0.7) +
  geom_text(aes(label = sprintf("%.2f", media_puntos)), 
            hjust = 1.1, size = 4, color = "white", fontface = "bold") +
  coord_flip() +
  labs(title = "Efectividad de Formaciones Tácticas",
       subtitle = "Puntos promedio por partido por formación (mínimo 5 partidos)",
       x = "Formación", y = "Puntos por Partido") +
  scale_fill_gradient(low = azul_claro, high = azul_oscuro, name = "Puntos/Partido") +
  theme_minimal() +
  theme(legend.position = "none")

g6 <- ggplot(formaciones_analisis, aes(x = media_gf, y = media_ga, size = partidos)) +
  geom_point(alpha = 0.7, color = azul_principal) +
  geom_text(aes(label = formation), vjust = -0.8, size = 3.5, color = gris_contraste) +
  labs(title = "Balance Ofensivo-Defensivo por Formación",
       subtitle = "Tamaño del punto: cantidad de partidos jugados",
       x = "Goles a Favor por Partido", 
       y = "Goles en Contra por Partido") +
  theme_minimal() +
  theme(legend.position = "none")
...
Interpretación Gráfico 5: Este gráfico muestra la eficiencia de las formaciones en puntos por partido. Algunas formaciones poco comunes, como el 4-1-2-1-2, el 3-5-2 y el 3-2-4-1, lograron un promedio de 3.00 puntos por partido. La formación 4-3-3 es la más usada. Tiene un promedio de 2.21 puntos por partido. Esta formación es la más frecuente entre las formaciones habituales. La elección sugiere que el cuerpo técnico busca un equilibrio en los resultados a largo plazo. Las formaciones con tres defensas, como el 3-5-2 y el 3-4-3, tienen un promedio de 2.44 puntos por partido. Son menos efectivas que un sistema con cuatro defensas, que es más confiable para obtener puntos.

Interpretación Gráfico 6: El Balance Ofensivo-Defensivo revela la calidad  de cada táctica. El 4-3-3 se destaca como la formación más popular y equilibrada, con un alto número de goles a favor y pocos en contra. La concentración de las formaciones más utilizadas en la zona de alto rendimiento sugiere que el equipo técnico está aprendiendo y ajustando constantemente su estrategia, eligiendo esquemas que realmente marcan la diferencia en el juego. Además, el 4-2-3-1 también se presenta como una opción sólida en términos de defensa.

```{r tabla4, echo=FALSE}
tabla_formaciones <- formaciones_analisis %>%
  select(formation, partidos, frecuencia_relativa, media_puntos, media_gf, media_ga, media_poss) %>%
  rename(
    'Formación' = formation,
    'Partidos' = partidos,
    'Frecuencia (%)' = frecuencia_relativa,
    'Puntos/Partido' = media_puntos,
    'GF/Partido' = media_gf,
    'GC/Partido' = media_ga,
    'Posesión (%)' = media_poss
  )

knitr::kable(tabla_formaciones, 
             caption = "Tabla 4: Análisis Comparativo de Formaciones Tácticas",
             digits = 2)
```


```{r tabla5, echo=FALSE}
conclusiones_mejoradas <- data.frame(
  Objetivo = c(
    "Evolución del modelo de juego",
    "Relación posesión-eficiencia", 
    "Efectividad de formaciones",
    "Rendimiento por localía",
    "Consistencia ofensiva"
  ),
  Hallazgo = c(
    ifelse(mean(tendencias_anuales$promedio_posesion, na.rm = TRUE) > 62, 
           "Mantenimiento del modelo de posesión alto", 
           "Cambio en el estilo de juego"),
    ifelse(cor(barca_data$poss, barca_data$xg, use = "complete.obs") > 0.3,
           "Posesión correlaciona con creación de oportunidades",
           "Posesión no garantiza mejor creación"),
    ifelse(nrow(formaciones_analisis) > 0 && max(formaciones_analisis$media_puntos, na.rm = TRUE) > 2.4,
           "Formaciones con alta efectividad identificadas",
           "Efectividad similar entre formaciones"),
    ifelse(nrow(rendimiento_venue) == 2 && rendimiento_venue$puntos_por_partido[1] > rendimiento_venue$puntos_por_partido[2],
           "Mejor rendimiento como local",
           "Rendimiento similar en ambos escenarios"),
    ifelse(mean(tendencias_anuales$sd_gf, na.rm = TRUE) < 1.2,
           "Alta consistencia ofensiva",
           "Irregularidad en producción ofensiva")
  ),
  Valor = c(
    paste(round(mean(tendencias_anuales$promedio_posesion, na.rm = TRUE), 1), "%"),
    paste(round(cor(barca_data$poss, barca_data$xg, use = "complete.obs"), 2)),
    ifelse(nrow(formaciones_analisis) > 0, 
           paste(round(max(formaciones_analisis$media_puntos, na.rm = TRUE), 2)),
           "-"),
    ifelse(nrow(rendimiento_venue) == 2,
           paste(round(abs(diff(rendimiento_venue$puntos_por_partido)), 2)),
           "-"),
    paste(round(mean(tendencias_anuales$sd_gf, na.rm = TRUE), 2))
  ),
  Implicación = c(
    "Confirmación identidad de juego",
    "Validación modelo táctico",
    "Base decisiones técnicas",
    "Mejorar rendimiento visitante",
    "Estabilidad en rendimiento"
  )
)

# Crear tabla formateada
knitr::kable(conclusiones_mejoradas, 
             caption = "Tabla 5: Conclusiones del Análisis Estadístico Descriptivo",
             col.names = c("Objetivo", "Hallazgo Principal", "Valor", "Implicación"),
             align = c("l", "l", "c", "l"))
```
...
El análisis estadístico del FC Barcelona en las temporadas 2020 a 2024 muestra cambios en su estilo de juego. El equipo jugó con una estrategia basada en mantener la posesión del balón. La posesión promedio fue del 64.9%. Sin embargo, tener más posesión no garantizó el éxito. La temporada 2023 fue la más exitosa. El equipo obtuvo 88 puntos y tuvo una efectividad del 77.2%. Esto se debió a una defensa fuerte, que permitió solo 0.53 goles en contra por partido. La ofensiva también fue eficiente. La posesión se mantuvo similar a años anteriores, pero no fue el factor principal para ganar partidos.


Se confirmó que la posesión del balón no se relaciona directamente con la efectividad del equipo. La clave está en cómo se usa esa posesión. Las formaciones más frecuentes fueron el 3-4-3 y el 3-2-4-1. Estas formaciones mostraron un equilibrio entre ataque y defensa. Otros sistemas con tres defensas tuvieron resultados diferentes. El equipo jugó bien como local, pero tuvo más dificultades como visitante. Esto indica que se puede mejorar en partidos fuera de casa. En general, el FC Barcelona superó una fase de cambio después de Messi. El equipo tuvo que cambiar tacticamente y mejorar la eficiencia. 