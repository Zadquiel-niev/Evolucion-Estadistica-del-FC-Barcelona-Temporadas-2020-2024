...
# convertir a numérico con limpieza
...
if col_poss:
    ...
    # si los valores están entre 0 y 1 (ej 0.66), multiplicar por 100
...
if not col_poss or not col_season:
    ...
    # ajustar rango eje y plausible (40-80) si datos lo permiten
    if not s1['Posesión promedio (%)'].isna().all():
        ymin = max(0, min(30, s1['Posesión promedio (%)'].min() - 2))
        ymax = max(100, s1['Posesión promedio (%)'].max() + 2)
        # for display, set autorange but suggest typical range if values plausible
...
if not col_poss or not col_xg:
    ...
    if scatter.empty:
        ...
        if len(scatter) > 1:
            ...
            xs = np.linspace(scatter['poss'].min(), scatter['xg'].max(), 100)
...
#GRAFICO 4: Puntos promedio por formación (barra)
...
if not col_result or not col_season:
    ...
    # etiquetas y colores 
    label_map = {'W':'Perdidos (P)', 'D':'Empatados (D)', 'L':'Ganados (W)'}
    df5_long['Etiqueta'] = df5_long['Resultado'].map(label_map)
    colors = {'Lose (L)':'#a50044', 'Empatados (D)':'#ffcc00', 'Ganados (W)':'#004d98'}
